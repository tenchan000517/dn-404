...
//         require(calAllowedAddresses[calLevel].contains(operator), "Operator not allowed");
...
//             console.log("  Current CAL Level:", calLevel);
//             console.log("  Is caller allowed:", _isAllowed(msg.sender, from));
//             console.log("  Is caller the owner:", msg.sender == from);
//             console.log("  Is caller approved for all:", isApprovedForAll(from, msg.sender));
//             console.log("  Is token or wallet locked:", isLocked(from, tokenId));
...
//         require(!isLocked(from, tokenId), "Token or wallet is locked");
...
//         require(_isAllowed(msg.sender, from) || msg.sender == from || isApprovedForAll(from, msg.sender), "Caller is not allowed to safeTransferFrom bytes");
//         require(!isLocked(from, tokenId), "Token or wallet is locked");
//         require(ownerOf(tokenId) == from, "From address is not the owner");
